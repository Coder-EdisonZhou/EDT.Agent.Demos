...
public class UniversalFunctionCaller
{
    ...
    public UniversalFunctionCaller(Kernel kernel)
    {
        _kernel = kernel;
    ...
    private async Task<FunctionCall?> GetNextFunctionCallAsync(ChatHistory history, string pluginsAsText)
    {
        ...
        {
            var systemMessage = GetLoopSystemMessage(pluginsAsText);
        ...
        for (int retryCount = 0; retryCount < 5; retryCount++)
        {
            ...
            try
            {
                ...
                var functionCall = ParseTextPrompt3000Call(llmReply);
            ...
            catch (Exception ex)
            {
                ...
                var errorMessage = $"Error: '{ex.Message}'. Please try again without apologizing or explaining. Just follow the previous rules.";
    ...
    {
        var systemPrompt = $@"You are an advisor, that is tasked with finding the next step to fullfil a goal.
...
";
    ...
    private async Task<FunctionCall> GetNextFunctionToCallDoubleTappedAsync(ChatHistory tap1history, string pluginsAsTextPrompt3000)
    {
        ...
        var tap2Prompt = @$"You are a computer system. You can only answer with a TextPrompt3000 function, nothing else. 
...
If you do not do this, a very cute kitten dies.";
        ...
        var tap2History = new ChatHistory();
        ...
        for (int iRetry = 0; iRetry < 5; iRetry++)
        {
            ...
            try
            {
                var functionCall = ParseTextPrompt3000Call(tap2Answer[0]);
                ValidateFunctionWithKernel(functionCall);
    ...
    private FunctionCall ParseTextPrompt3000Call(ChatMessageContent possibleFunctionCallResult)
    {
        ...
        var content = possibleFunctionCallResult.Content;

        // Split the content into function name and parameters
        var openParenIndex = content.IndexOf('(');
        var closeParenIndex = content.IndexOf(')');
        var functionName = content.Substring(0, openParenIndex);
        var parametersContent = content.Substring(openParenIndex + 1, closeParenIndex - openParenIndex - 1);
